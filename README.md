...
- **2026-04-08** 🧩 对齐密钥配置向导并恢复 OpenAI-compatible workflow 入口：支持自定义 workflow 平台与独立 rerank 配置，同时保留 BLT 默认链路兼容。
...
项目现已支持 **任意 OpenAI-compatible workflow 平台**，并支持将 **rerank 平台独立配置**。README 仍以 **柏拉图 API 平台** 作为兼容示例，你也可以替换为自己的第三方平台。

- 🌐 可选示例：[柏拉图 API 平台](https://api.bltcy.ai/)
- 📝 准备可用于 workflow 的 API Key / Base URL / Model
- 🔐 如需独立 rerank，可额外准备一组 rerank 平台配置
...
完成以上步骤后，后续大多数日常使用和配置都可以直接在网页端完成。若未填写独立 rerank，系统会自动走 fallback 排序，后续也可在网页端补充 rerank 平台配置。后续教程参考：[daily-paper-reader 指引](https://ziwenhahaha.github.io/daily-paper-reader/#/tutorial/README)
